...
export const ensureFichePhotoUrl = async ({ tableName, ficheId, clientId, photoUrl, isBar = false }) => {
  ...
  // Utiliser une signed URL (valide 1 an) plutôt que getPublicUrl,
  // afin que l'image soit accessible même si le bucket n'est pas public.
  const { data: signedData, error: signErr } = await supabase.storage
    .from('fiches-photos').createSignedUrl(repairedPath, 60 * 60 * 24 * 365)
  const repairedUrl = (!signErr && signedData?.signedUrl)
    ? signedData.signedUrl
    : (supabase.storage.from('fiches-photos').getPublicUrl(repairedPath).data?.publicUrl || photoUrl)
