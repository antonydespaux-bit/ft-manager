...
function extractStoragePathFromUrl(url, bucket) {
  ...
  try {
    // URL publique : /storage/v1/object/public/{bucket}/...
    const pubMarker = `/storage/v1/object/public/${bucket}/`
    let idx = url.indexOf(pubMarker)
    if (idx !== -1) return decodeURIComponent(url.slice(idx + pubMarker.length))

    // URL signée : /storage/v1/object/sign/{bucket}/...?token=...
    const signMarker = `/storage/v1/object/sign/${bucket}/`
    idx = url.indexOf(signMarker)
    if (idx !== -1) return decodeURIComponent(url.slice(idx + signMarker.length).split('?')[0])
...
// Vérifie que photo_url est accessible ; si non, tente de la réparer via getPublicUrl.
// Le bucket fiches-photos est public : pas besoin de signed URLs.
export const ensureFichePhotoUrl = async ({ tableName, ficheId, clientId, photoUrl, isBar = false }) => {
  ...
  const currentPath = extractStoragePathFromUrl(photoUrl, 'fiches-photos')
  ...
  const { data: files } = await supabase.storage
    .from('fiches-photos').list(clientId, { limit: 100, search: prefix })
  if (!files?.length) return photoUrl

  const item = files.find(f => f?.name?.startsWith(prefix)) || files[0]
  if (!item?.name) return photoUrl

  const repairedPath = `${clientId}/${item.name}`
