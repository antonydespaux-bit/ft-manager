...
export function buildRapportHtml({ data, debut, fin, commentaire, titre, articles, articlesVentes }) {
  ...
  const sections = [
    ...
    renderTmFoodBevLieux(data.tmLieux, periode),
...
// TM Food / Beverage détaillé par lieu (réutilise data.tmLieux, qui porte
// désormais real_tm_food / real_tm_bev par (lieu, service)). Une ligne Food +
// une ligne Beverage par lieu/service ayant des couverts.
function renderTmFoodBevLieux(tmLieux, periode) {
  if (!tmLieux || tmLieux.length === 0) return ''
  ...
  const items = tmLieux.map((r) => {
    const svc = SERVICE_LABEL[r.service]
    return (
      line(`Food ${r.lieu_label} ${svc}`, r.real_tm_food, r.budget_tm_food, r.ratio_food) +
      line(`Beverage ${r.lieu_label} ${svc}`, r.real_tm_bev, r.budget_tm_bev, r.ratio_bev)
    )
  }).join('')
  return `<h3 style="${styles.h3}">Ticket moyen Food et Beverage par lieu ${esc(periode)}</h3><ul style="${styles.ul}">${items}</ul>`
}
