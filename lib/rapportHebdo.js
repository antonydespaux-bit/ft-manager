...
export function tmParLieuService(caRows, budgetRows, lieuxMap, debut, fin, joursFermesIso = null, joursOverrideRows = null) {
  ...
  for (const r of filtered) {
    ...
    if (!realMap.has(key)) realMap.set(key, { couverts: 0, ca: 0, food: 0, bev: 0 })
    ...
    // Food / Beverage isolés (l'« Autre CA » reste hors food & bev).
    acc.food += Number(r.ca_food || 0)
    acc.bev += Number(r.ca_bev_20 || 0) + Number(r.ca_bev_10 || 0)
  }
  ...
  for (const d of eachDayIso(debut, fin)) {
    ...
    for (const cell of cells.values()) {
      ...
      if (!budgetMap.has(key)) budgetMap.set(key, { couverts_cible: 0, ca_cible: 0, food_cible: 0, bev_cible: 0 })
      ...
      acc.ca_cible +=
        ...
        Number(cell.ca_bev_10_cible || 0) +
        Number(cell.ca_autre_cible || 0)
      acc.food_cible += Number(cell.ca_food_cible || 0)
      acc.bev_cible += Number(cell.ca_bev_20_cible || 0) + Number(cell.ca_bev_10_cible || 0)
    }
  }
  ...
  for (const key of keys) {
    ...
    const real = realMap.get(key) || { couverts: 0, ca: 0, food: 0, bev: 0 }
    const budget = budgetMap.get(key) || { couverts_cible: 0, ca_cible: 0, food_cible: 0, bev_cible: 0 }
    ...
    // TM Food / Beverage isolés (même dénominateur couverts que le TM global).
    const tmRatio = (r, b) => {
      const rt = real.couverts > 0 ? r / real.couverts : null
      const bt = budget.couverts_cible > 0 ? b / budget.couverts_cible : null
      const rat = rt != null && bt != null && bt > 0 ? ((rt - bt) / bt) * 100 : null
      return { real_tm: rt, budget_tm: bt, ratio: rat }
    }
    const food = tmRatio(real.food, budget.food_cible)
    const bev = tmRatio(real.bev, budget.bev_cible)
    out.push({
      lieu_id: lieuId,
      ...
      delta_tm: delta,
      ratio_tm: ratio,
      real_tm_food: food.real_tm,
      budget_tm_food: food.budget_tm,
      ratio_food: food.ratio,
      real_tm_bev: bev.real_tm,
      budget_tm_bev: bev.budget_tm,
      ratio_bev: bev.ratio,
    })
  }
...
// `couvertsExclusIds` (optionnel) : Set d'ids de lieux (remappés parent) dont
// les couverts ne comptent pas dans cette section (lieux couverts_comptes=false).
export function couvertsParService(caRows, budgetRows, debut, fin, joursFermesIso = null, joursOverrideRows = null, couvertsExclusIds = null) {
  const exclus = couvertsExclusIds || null
  const filtered = filterCaJournalier(caRows, debut, fin)
  let realLunch = 0
  let realDinner = 0
  for (const r of filtered) {
    if (exclus && exclus.has(r.lieu_service_id)) continue
  ...
  for (const d of eachDayIso(debut, fin)) {
    ...
    for (const cell of cells.values()) {
      if (cell.jour_semaine !== d.isoJds) continue
      if (exclus && exclus.has(cell.lieu_service_id)) continue
      if (!isCellElectedForDate(cell, d.iso, annee, mois, electedMap)) continue
...
export function couvertsJourParJour(caRows, budgetRows, debut, fin, joursFermesIso = null, joursOverrideRows = null, couvertsExclusIds = null) {
  const exclus = couvertsExclusIds || null
  const filtered = filterCaJournalier(caRows, debut, fin)
...
export function buildRapportData({
  caRows,
  budgetRows,
  lieuxMap,
  debut,
  fin,
  joursFermesIso = null,
  joursOverrideRows = null,
  // Optionnels : lignes NON remappées vers le parent (1 ligne par lieu
  // physique) + libellés « self ». Réservés à la section TM par lieu, qui doit
  // détailler chaque lieu (Salle à manger, Table du chef, Privat…) au lieu de
  // les agréger sous leur parent. Les autres sections restent sur caRows/
  // budgetRows (remappés). Fallback = comportement historique.
  caRowsRaw = null,
  budgetRowsRaw = null,
  lieuxMapSelf = null,
  // Set d'ids de lieux (remappés parent) exclus de la SEULE section couverts
  // (lieux couverts_comptes=false, ex : Marsan Table de partage / La cave).
  couvertsExclusIds = null,
}) {
  ...
  const tmLieux = tmParLieuService(
    caRowsRaw ?? caRows,
    budgetRowsRaw ?? budgetRows,
    lieuxMapSelf ?? lieuxMap,
    debut,
    fin,
    joursFermesIso,
    joursOverrideRows
  ...
  const couverts = couvertsParService(caRows, budgetRows, debut, fin, joursFermesIso, joursOverrideRows, couvertsExclusIds)
  const couvertsJpJ = couvertsJourParJour(caRows, budgetRows, debut, fin, joursFermesIso, joursOverrideRows, couvertsExclusIds)
