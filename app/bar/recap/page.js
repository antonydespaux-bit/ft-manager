...
export default function BarRecapPage() {
    ...
    try {
      ...
      const [
        ...
        supabase.from('fiches_bar').select('*, lieux(id,nom,emoji), categories_plats(id,nom,emoji)')
          .eq('client_id', clientId).eq('archive', false).neq('categorie', 'Sous-fiche').order('nom'),
  ...
  const tvaFiche = (fiche) => CATEGORIES_ALCOOL.includes(fiche.categorie) ? 1.20 : 1.10

  const statsListe = (liste) => {
    if (!liste.length) return null
    const couts = liste.filter(f => f.cout_portion > 0).map(f => Number(f.cout_portion))
    const prixHTs = liste.filter(f => f.prix_ttc).map(f => f.prix_ttc / tvaFiche(f))
    const prixTTCs = liste.filter(f => f.prix_ttc).map(f => Number(f.prix_ttc))
    const benefices = liste.filter(f => f.prix_ttc && f.cout_portion).map(f => (f.prix_ttc / tvaFiche(f)) - Number(f.cout_portion))
    const ratios = liste.filter(f => f.prix_ttc && f.cout_portion).map(f => Number(f.cout_portion) / (f.prix_ttc / tvaFiche(f)) * 100)
  ...
  const fcColor = (fc) => { if (!fc) return c.texteMuted; if (fc < 22) return '#3B6D11'; if (fc < 28) return '#854F0B'; return '#A32D2D' }
  const fcBg = (fc) => { if (!fc) return 'transparent'; if (fc < 22) return '#EAF3DE'; if (fc < 28) return '#FAEEDA'; return '#FCEBEB' }
  ...
  const sauvegarderArchives = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('fiches_bar').update({ archive: true }).in('id', ids).eq('client_id', clientId)
      if (error) throw error
      await log({ action: 'ARCHIVAGE', entite: 'fiche_bar', entite_id: ids[0], entite_nom: `${ids.length} fiche(s) bar`, section: 'bar', details: `Archivage récap bar` })
      await loadData()
  ...
  const LigneFiche = ({ item }) => {
    const tva = tvaFiche(item)
    const cout = item.cout_portion
    ...
    return isMobile ? (
      <div style={{ background: aArchiver ? '#FAEEDA' : c.blanc, borderRadius: '8px', padding: '12px', marginBottom: '8px', border: `0.5px solid ${aArchiver ? '#FAC775' : c.bordure}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          <div style={{ fontSize: '14px', fontWeight: '500', color: c.texte, cursor: 'pointer', flex: 1 }} onClick={() => router.push(`/bar/fiches/${item.id}`)}>{item.nom}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {peutModifier && <input type="checkbox" checked={aArchiver} onChange={() => toggleArchive(item.id)} style={{ width: '18px', height: '18px', cursor: 'pointer', accentColor: '#7C3AED' }} />}
          </div>
        </div>
        <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: c.texteMuted, flexWrap: 'wrap' }}>
          ...
          {vue === 'categorie' && item.lieux && <span style={{ background: '#EEEDFE', color: '#3C3489', borderRadius: '20px', padding: '1px 8px', fontSize: '10px' }}>{item.lieux.emoji} {item.lieux.nom}</span>}
        </div>
      </div>
    ) : (
      <tr style={{ borderBottom: `0.5px solid ${c.bordure}`, background: aArchiver ? '#FAEEDA' : c.blanc }}>
        ...
        {vue === 'categorie' && (
          <td style={{ padding: '8px 10px' }}>
            {item.lieux && <span style={{ background: '#EEEDFE', color: '#3C3489', borderRadius: '20px', padding: '2px 8px', fontSize: '11px' }}>{item.lieux.emoji} {item.lieux.nom}</span>}
          </td>
        )}
        ...
        <td style={{ padding: '8px 10px', textAlign: 'right' }}>{item.prix_ttc ? `${Number(item.prix_ttc).toFixed(2)} €` : '—'}</td>
        ...
        {peutModifier && <td style={{ padding: '8px 10px', textAlign: 'right' }}><input type="checkbox" checked={aArchiver} onChange={() => toggleArchive(item.id)} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#7C3AED' }} /></td>}
      </tr>
    )
  }
    ...
    return (
      <div style={{ marginBottom: '8px' }}>
        <div onClick={() => setOuvert(isOpen ? null : id)} style={{
          background: isOpen ? '#EDE9FE' : c.blanc, borderRadius: isOpen ? '12px 12px 0 0' : '12px',
    ...
    <thead>
      <tr style={{ background: '#3C3489' }}>
        {['Lieu / Catégorie', 'Nb fiches', 'Coût moy./portion', 'Prix HT moy.', 'Prix TTC moy.', 'Bénéfice moy.', 'Ratio moy.'].map(col => (
          <th key={col} style={{ padding: '12px 16px', textAlign: col === 'Lieu / Catégorie' ? 'left' : 'right', fontSize: '11px', color: '#C4956A', fontWeight: '500', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{col}</th>
        ))}
      </tr>
    </thead>
  )
            ...
            <div key={lieu.id} style={{ marginBottom: '10px' }}>
              <div onClick={() => setOuvertLieu(isOpenLieu ? null : lieu.id)} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ background: '#EEEDFE', color: '#3C3489', borderRadius: '20px', padding: '4px 14px', fontSize: '12px', fontWeight: '500' }}>
                    {lieu.emoji} {lieu.nom}
                  </div>
              ...
              {isOpenLieu && (
                <div style={{ border: `0.5px solid ${c.bordure}`, borderTop: 'none', borderRadius: '0 0 12px 12px', background: c.fond, padding: '12px' }}>
                  {catsLieu.map(cat => {
                    ...
                    return (
                      <Accordeon
                        key={cat.id} id={`${lieu.id}-${cat.id}`}
                        badge={<span style={{ background: '#EDE9FE', color: '#3C3489', borderRadius: '20px', padding: '3px 12px', fontSize: '12px', fontWeight: '500' }}>{cat.emoji} {cat.nom}</span>}
                        stats={statsListe(fichesCat)} fiches={fichesCat}
                      />
                    )
                  })}
                  {(() => {
    ...
    return (
      <div>
        <div style={{ background: '#FEF3C7', border: '0.5px solid #FDE68A', borderRadius: '8px', padding: '10px 14px', marginBottom: '14px', fontSize: '13px', color: '#92400E' }}>
          Vue globale — toutes les fiches regroupées par type de boisson, quel que soit le lieu
        </div>
        {isMobile ? (
          <div>
            {catsAvecFiches.map(cat => {
              const fichesCat = fichesFiltrees.filter(f => f.categorie_plat_id === cat.id)
              return <Accordeon key={cat.id} id={`cat-${cat.id}`} badge={<span style={{ background: '#EDE9FE', color: '#3C3489', borderRadius: '20px', padding: '3px 12px', fontSize: '12px', fontWeight: '500' }}>{cat.emoji} {cat.nom}</span>} stats={statsListe(fichesCat)} fiches={fichesCat} avecLieu={true} />
            })}
                    ...
                    <>
                      ...
                      >
                        <td style={{ padding: '14px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <span style={{ background: '#EDE9FE', color: '#3C3489', borderRadius: '20px', padding: '3px 12px', fontSize: '12px', fontWeight: '500' }}>{cat.emoji} {cat.nom}</span>
                            <span style={{ fontSize: '11px', color: c.texteMuted }}>{stats.nb} fiche{stats.nb > 1 ? 's' : ''}</span>
                            <span style={{ fontSize: '11px', color: '#7C3AED', marginLeft: 'auto' }}>{isOpen ? '▲' : '▼'}</span>
                          </div>
                        </td>
            ...
            <thead>
              <tr style={{ background: '#3C3489' }}>
                {['Lieu', 'Fiches', 'Coût moy.', 'Prix TTC moy.', 'Bénéfice moy.', 'Food cost'].map(col => (
                  <th key={col} style={{ padding: '12px 16px', textAlign: col === 'Lieu' ? 'left' : 'right', fontSize: '11px', color: '#C4956A', fontWeight: '500', textTransform: 'uppercase' }}>{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={lieu.id} style={{ borderBottom: `0.5px solid ${c.bordure}` }}>
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ background: '#EDE9FE', color: '#3C3489', borderRadius: '20px', padding: '3px 12px', fontSize: '12px', fontWeight: '500' }}>{lieu.emoji} {lieu.nom}</span>
                    </td>
  ...
  return (
    <div style={{ minHeight: '100vh', background: c.fond }}>
      ...
      <div style={{ padding: isMobile ? '12px' : '24px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: '4px', background: c.blanc, padding: '3px', borderRadius: '8px', border: `0.5px solid ${c.bordure}` }}>
            {[{ id: 'lieu', label: '🏠 Par lieu' }, { id: 'categorie', label: '🍷 Par catégorie' }, { id: 'global', label: '📊 Global' }].map(v => (
              <button key={v.id} onClick={() => { setVue(v.id); setOuvert(null) }} style={{
                ...
                background: vue === v.id ? '#3C3489' : 'transparent',
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center' }}>
            {peutModifier && nbArchives > 0 && (
              <button onClick={sauvegarderArchives} disabled={saving} style={{ padding: '8px 16px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', border: 'none', cursor: saving ? 'not-allowed' : 'pointer', background: saving ? '#A5B4FC' : '#7C3AED', color: 'white' }}>
        ...
        {peutModifier && nbArchives > 0 && (
          <div style={{ background: '#EDE9FE', border: '0.5px solid #DDD6FE', borderRadius: '10px', padding: '10px 16px', marginBottom: '16px', fontSize: '13px', color: '#7C3AED', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span>📥 {nbArchives} fiche{nbArchives > 1 ? 's' : ''} sélectionnée{nbArchives > 1 ? 's' : ''} pour archivage</span>
            <button onClick={() => setModifArchive({})} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#7C3AED', fontSize: '13px', fontWeight: '500' }}>Annuler</button>
          </div>
        )}
